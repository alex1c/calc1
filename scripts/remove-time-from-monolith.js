...
try {
	...
	let deletedCount = 0;
	for (const key of timeCalculators) {
	...
	console.log('\n✅ Все калькуляторы Время успешно удалены из монолита!');

} catch (error) {
	console.error('❌ Произошла ошибка при удалении калькуляторов Время из монолита:', error);
}
