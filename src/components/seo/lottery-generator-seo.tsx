...
export default function LotteryGeneratorSEO() {
	const t = useTranslations('calculators.lotteryGenerator.seo');
						...
						{[0, 1, 2, 3, 4, 5, 6, 7].map((index) => (
							...
							>
								<h3 className='text-lg font-semibold text-gray-900 mb-2'>
									{t(`types.items.${index}.title`)}
								</h3>
								<p className='text-gray-700 text-sm mb-2'>
									{t(`types.items.${index}.description`)}
								</p>
								<div className='text-xs text-gray-600'>
									{t(`types.items.${index}.format`)}
								</div>
							</div>
						))}
					</div>
				...
				<section className='prose prose-lg max-w-none'>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
						{[0, 1, 2, 3].map((index) => (
							<div
								key={index}
								className='flex items-start gap-4 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-lg p-6 border border-yellow-100'
							>
