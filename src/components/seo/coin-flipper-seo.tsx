...
export default function CoinFlipperSEO() {
	const t = useTranslations('calculators.coinFlipper.seo');
	// FAQ is at root level, not inside seo
	const tRoot = useTranslations('calculators.coinFlipper');

	// Get FAQ items for structured data
	const faqRaw = tRoot.raw('faq.faqItems');
		...
		<div className='space-y-12'>
			...
			{t.raw('overview.calculationExamples') &&
				...
				t.raw('overview.calculationExamples').length > 0 && (
					<section className='bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20 rounded-xl shadow-lg p-8'>
						...
						<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
							{(
								...
								<div
									key={index}
									className='bg-white dark:bg-gray-800 rounded-lg p-6 shadow-md border border-yellow-200 dark:border-yellow-800'
								>
			...
			<section className='prose prose-lg max-w-none'>
				...
				<ul className='space-y-3'>
					{[1, 2, 3, 4, 5].map((index) => (
						<li
							key={index}
							className='flex items-start gap-3 bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20 rounded-lg p-4 border border-yellow-100 dark:border-yellow-800'
						>
							<span className='text-yellow-600 dark:text-yellow-400 mt-1'>
								🪙
							</span>
							<span className='text-gray-700 dark:text-gray-300'>
								{t(`features.feature${index}`)}
							</span>
						</li>
					))}
				</ul>
