...
import {
	...
	Zap,
	RefreshCcw,
	// Handshake icon may be unavailable in some lucide versions; use HelpCircle instead
	// Handshake,
	Info,
	HelpCircle,
	Car,
...
export default function CustomsSEO() {
	...
	const tSeo = useTranslations('calculators.customs.seo');
	...
	// Chart data for visualization - Engine volume impact
	const engineVolumeImpact = [
		{
			volume: 'До 1000 см³',
			rate: 1.5,
			cost: 15000,
	...
	// Chart data for visualization - Age impact
	const ageImpact = [
		{
			age: 'До 3 лет',
			multiplier: 1.0,
			description: 'Базовые ставки',
			color: 'bg-green-500',
		},
		{
			age: 'Старше 3 лет',
			multiplier: 2.0,
			description: 'Удвоенные ставки',
			color: 'bg-red-500',
		},
	];

	const maxCost = Math.max(...engineVolumeImpact.map((item) => item.cost));
			...
			<div className='bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 md:p-8'>
				...
				<div>
					...
					<div className='mt-8 grid grid-cols-1 md:grid-cols-2 gap-6'>
						{/* Engine Volume Impact */}
						<div className='bg-gray-50 dark:bg-gray-700 p-5 rounded-lg'>
							<h4 className='text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2'>
								<Settings className='w-5 h-5 text-blue-600 dark:text-blue-400' />
								Влияние объёма двигателя на стоимость растаможки
							</h4>
							<div className='space-y-3'>
								{engineVolumeImpact.map((item, index) => (
									<div key={index}>
						...
						<div className='bg-gray-50 dark:bg-gray-700 p-5 rounded-lg'>
							<h4 className='text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2'>
								...
								Влияние возраста автомобиля на акциз
							</h4>
							<div className='space-y-3'>
								{ageImpact.map((item, index) => (
									<div key={index}>
										<div className='flex justify-between mb-1'>
											<div>
												<span className='text-sm font-medium text-gray-700 dark:text-gray-300'>
				...
				<div className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mt-4'>
					<ul className='list-disc list-inside space-y-2 ml-4'>
						<li className='text-gray-700 dark:text-gray-300'>
							{tSeo('coefficients.duty')}
						</li>
						<li className='text-gray-700 dark:text-gray-300'>
							{tSeo('coefficients.excise')}
						</li>
						<li className='text-gray-700 dark:text-gray-300'>
							{tSeo('coefficients.exciseOld')}
						</li>
						<li className='text-gray-700 dark:text-gray-300'>
							{tSeo('coefficients.vat')}
						</li>
						<li className='text-gray-700 dark:text-gray-300'>
							{tSeo('coefficients.recycling')}
						</li>
					</ul>
				</div>
			</div>
