...
export default function FoundationSEO() {
	const t = useTranslations('calculators.foundation');
	const tSeo = useTranslations('calculators.foundation.seo');
	...
	// Foundation types data for visualization
	const foundationTypes = [
		{ name: 'Ленточный', volume: 15, color: 'bg-blue-500' },
		{ name: 'Плитный', volume: 20, color: 'bg-green-500' },
		{ name: 'Столбчатый', volume: 8, color: 'bg-purple-500' },
	];

	// Concrete grade vs consumption data
	const concreteGrades = [
		{ grade: 'М200', cement: 280, color: 'bg-blue-500' },
		{ grade: 'М250', cement: 330, color: 'bg-green-500' },
		{ grade: 'М300', cement: 380, color: 'bg-purple-500' },
	];
			...
			<div className='bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6'>
				...
				<div>
					...
					<div className='mt-8 grid grid-cols-1 md:grid-cols-2 gap-6'>
						{/* Foundation Types Chart */}
						<div className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg'>
							<h4 className='text-lg font-semibold text-gray-900 dark:text-white mb-4'>
								Объём бетона по типам фундамента (м³, пример)
							</h4>
						...
						{/* Concrete Grade Chart */}
						<div className='bg-gray-50 dark:bg-gray-700 p-4 rounded-lg'>
							<h4 className='text-lg font-semibold text-gray-900 dark:text-white mb-4'>
								Расход цемента по маркам бетона (кг/м³)
							</h4>
							<div className='space-y-3'>
								{concreteGrades.map((grade, index) => (
									<div key={index}>
										...
										<div className='w-full bg-gray-200 dark:bg-gray-600 rounded-full h-4'>
											<div
												className={`${grade.color} h-4 rounded-full flex items-center justify-end pr-2`}
												style={{
													width: `${
														(grade.cement / 380) *
														100
				...
				<div>
					...
					<div className='bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg'>
						...
						<ul className='space-y-2 text-yellow-800 dark:text-yellow-200 text-sm'>
							<li>
								<strong>Ленточный:</strong>{' '}
								{tSeo('calculation.strip')}
							</li>
							<li>
								<strong>Плитный:</strong>{' '}
								{tSeo('calculation.slab')}
							</li>
							<li>
								<strong>Столбчатый:</strong>{' '}
								{tSeo('calculation.columnar')}
							</li>
							<li>
								<strong>Арматура:</strong>{' '}
								{tSeo('calculation.rebar')}
							</li>
							<li>
								<strong>Материалы:</strong>{' '}
								{tSeo('calculation.materials')}
							</li>
						</ul>
					</div>
				</div>
					...
					<div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
						<div className='bg-green-50 dark:bg-green-900/20 p-4 rounded-lg'>
							...
							<ul className='text-sm text-green-800 dark:text-green-200 space-y-1'>
								<li>• {tSeo('tips.preparation.soil')}</li>
								<li>• {tSeo('tips.preparation.level')}</li>
								<li>• {tSeo('tips.preparation.drainage')}</li>
							</ul>
						</div>
						<div className='bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg'>
							...
							<ul className='text-sm text-orange-800 dark:text-orange-200 space-y-1'>
								<li>• {tSeo('tips.reinforcement.diameter')}</li>
								<li>• {tSeo('tips.reinforcement.spacing')}</li>
								<li>• {tSeo('tips.reinforcement.layers')}</li>
							</ul>
						</div>
						<div className='bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg'>
							...
							<ul className='text-sm text-purple-800 dark:text-purple-200 space-y-1'>
								<li>• {tSeo('tips.concreting.grade')}</li>
								<li>• {tSeo('tips.concreting.mixing')}</li>
								<li>• {tSeo('tips.concreting.curing')}</li>
							</ul>
						</div>
					</div>
				</div>
